...
class HelpersTest extends TestCase
{
    ...
    {
        $parsed = parseEnvFile($this->makeEnvFile());
        $correctData = [
            "TZ" => "Asia/Kuwait",
            PF . "CRON_IMPORT" => "1",
            PF . "CRON_EXPORT" => "0",
            PF . "CRON_IMPORT_AT" => "16 */1 * * *",
            PF . "CRON_EXPORT_AT" => "30 */3 * * *",
            PF . "CRON_PUSH_AT" => "*/10 * * * *",
        ];
    ...
    public function test_loadEnvFile(): void
    {
        ...
        loadEnvFile($this->makeEnvFile(), usePutEnv: true, override: false);
    ...
    protected function makeEnvFile(): string
    {
        // -- test data.
        $data = [
            'TZ=Asia/Kuwait',
        ...
        $envFile = tempnam(sys_get_temp_dir(), 'test-env');
        $f = new Stream(fopen($envFile, 'w'));
        foreach ($data as $line) {
            $f->write($line . PHP_EOL);
        }
        return $envFile;
    }
}
